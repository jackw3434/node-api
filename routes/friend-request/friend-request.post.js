...
module.exports = function (router) {
    router.route('/friendRequest').post(auth.optional, function (req, res) {
        ...
        User.find({ 'email': { $in: [friendToInvite, sender] } }, function (err, users) {
            ...
            //mongoose.Types.ObjectId(requestSender.friend_requests)

            // FriendRequest.find({ '_id': { $in: [requestSender.friend_requests] } }, function (err, foundFriendRequests) {

            //     if (err) {
            //         console.log("sdfsdf ", err);
            //         return res.status(400).send('Generic_Error, getFriendRequest_failed, Unable to Get FriendRequest');
            //     }

            //     foundFriendRequests.map((eachFriendRequest) => {

            //         if (eachFriendRequest.receiver == friendToInvite) {
            //             console.log("request already exists", eachFriendRequest);
            //             return res.status(400).send('Validation_error, A Friend Request to ' + friendToInvite + ' has already been made.');
            //         } else {
            //             console.log("OK");
            //         }
            //     });
